...
def test_compress_directory(
    ...
    # Create config
    config = PngquantConfig(quality_min=65, quality_max=80, speed=1)
    ...
    summary = compress_images(
        raw_paths=rel_paths, run_id="test_run", config=config, obsidian_dir=input_path
    )
...
if __name__ == "__main__":
    ...
    for raw_file in raw_files:
        ...
        assert processed_file.exists(), (
            f"Processed file {processed_file} does not exist for raw file {raw_file}"
        )
        assert processed_file.stat().st_size > 0, (
            f"Processed file {processed_file} is empty"
        )
    logger.success("Processed files exist correctly")
